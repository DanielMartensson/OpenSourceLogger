...
#include <chrono>
#include <string>
#include <filesystem> // At least C++17
#include <sstream>
...
bool showFileDialog(bool* file_dialog_open, char* buffer, unsigned int buffer_size, FileDialogType type) {
	...
	if (*file_dialog_open) {
		...
		// Delete file button
		static bool disable_delete_file_button = false;
		disable_delete_file_button = (file_dialog_current_file == "");
		if (disable_delete_file_button) {
		...
		static bool disable_delete_folder_button = false;
		disable_delete_folder_button = (file_dialog_current_folder == "");
		if (disable_delete_folder_button) {
			ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true);
			ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f);
		}
		...
		if (ImGui::BeginPopup("NewFilePopup", ImGuiWindowFlags_Modal)) {
			ImGui::Text("Enter a name for the new file");
			static char new_file_name[500] = "";
			static char new_file_error[500] = "";
			ImGui::InputText("##newfile", new_file_name, sizeof(new_file_name));
			if (ImGui::Button("Create##1")) {
				if (strlen(new_file_name) <= 0) {
					strcpy_s(new_file_error, "File name can't be empty");
				}
				else {
					std::string new_file_path = file_dialog_current_path + (file_dialog_current_path.back() == '\\' ? "" : "\\") + new_file_name;
			...
			if (ImGui::Button("Cancel##1")) {
				strcpy_s(new_file_name, "");
				strcpy_s(new_file_error, "");
				ImGui::CloseCurrentPopup();
			}
			ImGui::TextColored(ImColor(1.0f, 0.0f, 0.2f, 1.0f), new_file_error);
			ImGui::EndPopup();
		}
		...
		if (ImGui::BeginPopup("DeleteFilePopup", ImGuiWindowFlags_Modal)) {
			ImGui::TextColored(ImColor(1.0f, 0.0f, 0.2f, 1.0f), "Are you sure you want to delete this file?");
			ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 6);
			ImGui::TextUnformatted(file_dialog_current_file.c_str());
			ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 6);
			if (ImGui::Button("Yes")) {
				std::filesystem::remove(file_dialog_current_path + (file_dialog_current_path.back() == '\\' ? "" : "\\") + file_dialog_current_file);
				ImGui::CloseCurrentPopup();
			}
			ImGui::SameLine();
			if (ImGui::Button("No")) {
